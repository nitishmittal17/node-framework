...
class RouterConfigurer {
    ...
    preHandler(request, response, next) {
        return __awaiter(this, void 0, void 0, function* () {
            response.on('finish', () => __awaiter(this, void 0, void 0, function* () {
                for (let i = this.interceptors.length - 1; i >= 0; i -= 1) {
                    ...
                    if (_.isFunction(interceptor.afterCompletion)) {
                        try {
                            yield interceptor.afterCompletion(request, response);
                        }
                        catch (err) {
                            logger.error(`After completition failed on ${request.method} ${request.url}. Interceptor: ` +
                                `${ComponentDecorator_1.ComponentUtil.getComponentData(interceptor.constructor).componentName}`, err);
                        }
                    }
                }
            }));
            for (let i = 0; i < this.interceptors.length; i += 1) {
                ...
                if (_.isFunction(interceptor.preHandle)) {
                    ...
                    try {
                        if ((yield interceptor.preHandle(request, response)) === false) {
                            return;
                        }
                    }
                    catch (err) {
